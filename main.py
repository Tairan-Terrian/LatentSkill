...
def _extract_memory_actions_step(trainer: BaseTrainer,
    ...
    action_idx, _, _, _ = trainer.controller(state_tensor, op_tensor, deterministic=True)
...
def _build_raw_session_memory_bank(trainer: BaseTrainer, sessions) -> MemoryBank:
    """Build a source-backed memory bank that preserves original chunks verbatim."""
    session_texts = sessions if isinstance(sessions, list) else list(sessions)
    memory_bank = trainer._initialize_memory_bank()
    if not session_texts:
        return memory_bank

    retriever_embeddings = get_embeddings(
        trainer.args.retriever,
        session_texts,
        "context"
    )
    state_encoder_embeddings = trainer.memory_bank_encoder._encode_texts(session_texts)
    if hasattr(retriever_embeddings, "ndim") and retriever_embeddings.ndim == 1:
        retriever_embeddings = retriever_embeddings.reshape(1, -1)
    if hasattr(state_encoder_embeddings, "ndim") and state_encoder_embeddings.ndim == 1:
        state_encoder_embeddings = state_encoder_embeddings.reshape(1, -1)

    memory_bank.initialize_from_sessions(
        session_texts,
        retriever_embeddings,
        state_encoder_embeddings=state_encoder_embeddings
    )
    return memory_bank


def _merge_memory_banks(primary: MemoryBank, secondary: MemoryBank) -> MemoryBank:
    """Append memories from secondary into primary, keeping existing embeddings."""
    for item in getattr(secondary, "memories", []):
        primary.add_memory(
            content=item.content,
            embedding=item.embedding,
            metadata=dict(item.metadata or {}),
            operation_name=item.operation_history,
            state_encoder_embedding=item.state_encoder_embedding
        )
    return primary
...
def _memory_cache_path(
    ...
    eval_memory_source = str(getattr(args, 'eval_memory_source', 'policy') or 'policy')
    if eval_memory_source != 'policy':
        name_parts.append(f"memsrc_{_sanitize_filename(eval_memory_source)}")
...
def evaluate_text_dataset_queries(trainer: BaseTrainer,
    ...
    for conv_idx, conversation in enumerate(test_data):
        ...
        for idx, (qa_idx, qa) in enumerate(valid_qa):
            ...
            retrieval_mode = str(getattr(args, "eval_retrieval_mode", "semantic") or "semantic")
            if retrieval_mode == "hybrid_text":
                retrieved_mems, retrieved_indices = memory_bank.retrieve_hybrid_text(
                    question,
                    q_embedding,
                    top_k=top_k_eval,
    ...
    for category, data in category_metrics.items():
        category_summary[str(category)] = {
            "count": len(data.get("f1", [])),
            "f1": _avg(data.get("f1", [])),
            "llm_judge": _avg(data.get("llm_judge", [])),
        }
    ...
    query_results = []
    for qid in sorted(meta_by_qid.keys()):
        meta = meta_by_qid.get(qid, {})
        qa = meta.get("qa", {})
        ground_truth = trainer.evaluator.get_ground_truth(qa)
        query_results.append({
            ...
            "prediction": predictions.get(qid, ""),
            "f1": float(
                trainer.evaluator.compute_f1(
                    predictions.get(qid, ""),
                    ground_truth,
                    qa
                )
            ),
            "llm_judge": float(llm_judge_scores.get(qid, 0.0)),
            "retrieved_indices": meta.get("retrieved_indices", []),
            "retrieved_memories": meta.get("retrieved_memories", []),
        })

    return {
        "dataset": args.dataset,
        "num_queries": len(metrics["f1"]),
        "metrics": {
            "f1": _avg(metrics["f1"]),
...
def infer_text_dataset_memories(trainer: BaseTrainer, test_data, args):
    ...
    def _compute_single_sample_memory(conversation, sample_id: str, memory_path: str, use_local_executor: bool):
        ...
        if eval_memory_source == "raw_sessions":
            memory_bank = _build_raw_session_memory_bank(trainer, sessions)
        ...
        if eval_memory_source == "hybrid":
            raw_bank = _build_raw_session_memory_bank(trainer, sessions)
            memory_bank = _merge_memory_banks(memory_bank, raw_bank)
        return sample_id, memory_path, memory_bank
    ...
    eval_result = evaluate_text_dataset_queries(trainer, test_data, memory_banks, args)
    eval_result["memory_dir"] = memory_dir
    eval_result["memory_banks"] = {
        "loaded": loaded_count,
        "computed": computed_count,
        "sample_ids": sorted(memory_banks.keys()),
    }
    return eval_result
...
def infer_alfworld_memories(trainer: BaseTrainer, train_data, test_data, args):
    ...
    return {
        ...
        "metrics": {
            ...
            "avg_interaction_steps": avg_steps,
        },
        "episodes": results,
    }
...
def main():
    ...
    if args.eval_only:
        ...
        if eval_result is not None:
            ...
            print(f"Saved evaluation results to {args.out_file}")
