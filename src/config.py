...
class AgenticMemoryConfig:
    def __init__(self):
        ...
        self.latent_dim = 64  # latent score dimension for structural decoder policy
        self.latent_log_std_init = -0.5  # init log std for latent Gaussian policy
        ...
        self.eval_memory_source = "policy"  # policy, raw_sessions, or hybrid
        self.eval_retrieval_mode = "semantic"  # semantic or hybrid_text
...
def get_agentic_memory_args():
    ...
    parser.add_argument('--latent-dim', type=int, default=64,
                        help='Latent score dimension for structural decoder policy')
    parser.add_argument('--latent-log-std-init', type=float, default=-0.5,
                        help='Initial log std for latent Gaussian policy')
    ...
    parser.add_argument('--eval-memory-source', type=str, default='policy',
                        choices=['policy', 'raw_sessions', 'hybrid'],
                        help='Memory source during eval-only inference: policy extraction, raw sessions, or both')
    parser.add_argument('--eval-retrieval-mode', type=str, default='semantic',
                        choices=['semantic', 'hybrid_text'],
                        help='Retrieval mode for text QA evaluation')
