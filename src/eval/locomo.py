...
class LoCoMoEvaluator(Evaluator):
    ...
    def build_prompt(self, question: str, retrieved_memories: List[str],
        ...
        extra = ""
        if category == 2 or any(word in question.lower() for word in ["when", "year", "month", "date"]):
            extra = (
                "\nTemporal instruction: Use the DATE line before each conversation. "
                "Resolve relative phrases such as tomorrow, yesterday, last week, next month, "
                "or a few years ago into the specific date/month/year when the context supports it."
            )
        elif question.lower().startswith("how many"):
            extra = "\nCounting instruction: answer with only the count, preferably a number or one short word."

        return (
            f"{context}\n\n"
            "Answer the question using only the conversation history above.\n"
            "Return the shortest exact answer possible: a name, object, place, count, date, month, year, or brief phrase.\n"
            "Do not write a full sentence. Do not explain. If multiple items are required, separate them with commas."
            f"{extra}\n\n"
            f"Question: {question}\n"
            "Short answer:"
        )
