...
class PPOBuffer:
    ...
    def push(self, state_emb: np.ndarray, op_embs: np.ndarray,
             action: Union[int, List[int]], latent_z: np.ndarray,
             log_prob: float, value: float, reward: float = 0.0):
        """
        ...
        self.latents.append(np.array(latent_z, dtype=np.float32))
    ...
    def merge(self, other: 'PPOBuffer'):
        ...
        self.latents.extend(other.latents)
    ...
    def get_batch(self) -> Dict:
        ...
        return {
            ...
            'latents': np.array(self.latents, dtype=np.float32),
...
class PPOController(nn.Module):
    """
    PPO Controller with latent-space actor-critic and structural decoder.
    - Actor (Policy): outputs latent Gaussian policy pi(z | s)
    - Decoder: maps sampled z to feasible operation(s) via score maximization
    - Critic (Value): outputs state value V(s)
    - PPO ratio/entropy are computed on latent density, not decoded actions
    """
    def __init__(self, state_dim: int = 768, op_dim: int = 768,
        ...
        # Actor in latent space: mean and (state-independent) log-std for pi(z|s)
        ...
        self.actor_log_std = nn.Parameter(
            torch.full((latent_dim,), float(latent_log_std_init), dtype=torch.float32)
        )

        # Structural decoder score terms: psi_s(a) + <z, F_s(a)>
        self.decoder_base_head = nn.Sequential(
            nn.Linear(hidden_dim * 2, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, 1)
        )
        self.decoder_feature_head = nn.Linear(hidden_dim, latent_dim)

        # Optional projection for feature stability in dot product term
        self.decoder_feature_norm = nn.LayerNorm(latent_dim)

        # Temperature for approximate action diagnostics (softmax over decoder scores)
        self.decoder_temp = 1.0

        # Keep for backward compatibility with external access
        self.actor_head = self.decoder_base_head
    ...
    def _sample_latent(self, state_h: torch.Tensor,
                       deterministic: bool = False) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Sample latent z ~ pi(z|s) (or use mean for deterministic mode).

        Returns:
            z: [B, latent_dim]
            log_prob: [B] log pi(z|s)
            entropy: [B] latent policy entropy
        """
        mean = self.actor_mean_head(state_h)  # [B, latent_dim]
    ...
    def _decode_action_scores(self, state_h: torch.Tensor, op_h: torch.Tensor,
                              latent_z: torch.Tensor,
                              mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Compute structural decoder scores for each operation:
            score(s, a; z) = psi_s(a) + <z, F_s(a)>

        state_h: [B, hidden_dim]
        op_h: [B, num_ops, hidden_dim]
        latent_z: [B, latent_dim]
        mask: [B, num_ops] - 1 for valid ops, 0 for padding
        Returns: [B, num_ops] scores
        """
        ...
        # Base structural term psi_s(a)
        combined = torch.cat([state_expanded, op_h], dim=-1)  # [B, num_ops, 2H]
        base_scores = self.decoder_base_head(combined).squeeze(-1)  # [B, num_ops]

        # Feature term <z, F_s(a)>
        features = self.decoder_feature_head(op_h)  # [B, num_ops, latent_dim]
        features = self.decoder_feature_norm(features)
        latent_expanded = latent_z.unsqueeze(1)  # [B, 1, latent_dim]
        dot_scores = (features * latent_expanded).sum(dim=-1)  # [B, num_ops]
        logits = base_scores + dot_scores
    ...
    def forward(self, state_embedding: torch.Tensor,
                ...
                ) -> Tuple[Union[int, List[int]], float, float, np.ndarray]:
        """
        ...
        Returns:
            ...
            - log_prob: float, latent log probability log pi(z|s)
            - value: float, state value estimate
            - latent_z: np.ndarray, sampled latent score vector
        """
        # Add batch dimension
        ...
        # Sample latent z and decode scores
        latent_z, latent_log_prob, _ = self._sample_latent(state_h, deterministic=deterministic)
        logits = self._decode_action_scores(state_h, op_h, latent_z)  # [1, num_ops]
        ...
        if k == 1:
            ...
            return action, latent_log_prob.item(), value.item(), latent_z[0].detach().cpu().numpy()
        else:
            _, top_k_indices = torch.topk(logits, k)
            actions = top_k_indices.tolist()
            return actions, latent_log_prob.item(), value.item(), latent_z[0].detach().cpu().numpy()

    def evaluate_actions(self, state_embs: torch.Tensor, op_embs: torch.Tensor,
                         latents: torch.Tensor, actions: torch.Tensor,
                         op_masks: Optional[torch.Tensor] = None
        ...
        Returns:
            log_probs: [B] latent log probabilities log pi(z_t | s_t)
        ...
        # Evaluate latent density under current policy
        mean = self.actor_mean_head(state_h)
        log_std = self.actor_log_std.unsqueeze(0).expand_as(mean)
        std = torch.exp(log_std)
        latent_dist = torch.distributions.Normal(mean, std)
        log_probs = latent_dist.log_prob(latents).sum(dim=-1)
        entropy = latent_dist.entropy().sum(dim=-1)

        # Decode scores for diagnostics and selected-action stats
        logits = self._decode_action_scores(state_h, op_h, latents, mask=op_masks)  # [B, max_ops]
    ...
    def compute_ppo_loss(self, batch: Dict, returns: np.ndarray,
        ...
        latents = torch.FloatTensor(np.array(batch['latents'], dtype=np.float32)).to(self.device)
        ...
        new_log_probs, values, entropy, topk_stats = self.evaluate_actions(
            state_embs, op_embs, latents, actions, op_masks
        )
        ...
        with torch.no_grad():
            if returns_t.numel() > 1:
                explained_var = 1 - (
                    (returns_t - values).var(unbiased=False)
                    / (returns_t.var(unbiased=False) + 1e-8)
                )
            else:
                explained_var = torch.tensor(0.0, device=returns_t.device)
